...
from .BayesianLayer import BayesianLayer


#class GraphConvolutionLayer_Channel(nn.Module): #channlewise graph convolution

class BayesianGraphConvolutionLayer(nn.Module): #it learns edge dependencies for human pose

    def __init__(self, in_features, out_features, adj, bias=True):
        super(BayesianGraphConvolutionLayer, self).__init__()
        self.in_features = in_features
        self.out_features = out_features

        self.bn1 = BayesianLayer(in_features,out_features, bias = False)
        self.bn2 = BayesianLayer(in_features,out_features, bias = False)

        self.adj = adj
        self.mask = (self.adj > 0)
        self.e = nn.Parameter(torch.zeros(1, len(self.mask.nonzero(as_tuple = False)), dtype=torch.float)) #weights of each joints
    ...
    def forward(self, input):
        h0 = self.bn1(input.clone())
        h1 = self.bn2(input.clone())
    ...
    def kl_divergence(self):
        return self.bn1.kl_divergence() + self.bn2.kl_divergence()
...
class GraphConvolutionLayer(nn.Module): #it learns edge dependencies for human pose
    ...
    def __init__(self, in_features, out_features, adj, bias=True):
        ...
        self.adj = adj
        self.mask = (self.adj > 0)
        self.e = torch.zeros(1, len(self.mask.nonzero(as_tuple = False)), dtype=torch.float) #weights of each joints
        e = adj[self.mask]
        e = nn.Parameter(e)
    ...
    def forward(self, input):
        ...
        adj = -9e15 * torch.ones_like(self.adj).to(input.device)
        adj[self.mask] = self.e.clone().to(input.device)
        adj = F.softmax(adj, dim=1)
