...
class MyLoss(nn.Module):
    ...
    def get_keypoints_loss(self, output, target):
    ...
    def forward(self, pred, gt):
        ...
        pred_smpl_keypoints = pred['pred_keypoints']
        ...
        gt_smpl_keypoints = gt['smpl_keypoints']
        gt_cam_t = gt['gt_cam_t']


        #keypoints loss
        loss_smpl_keypoints = self.get_keypoints_loss(pred_smpl_keypoints, gt_smpl_keypoints)
        ...
        loss_cam_t = 0.0005*torch.sum((pred_cam_t - gt_cam_t)**2,1).mean()
        loss_var = pred['loss_var']

        '''
        print(loss_shape)
        print(loss_keypoints)
        print(loss_keypoints_3d)
        print(loss_regr_pose)
        print(loss_regr_betas)
        print(loss_cam)
        print(((torch.exp(-pred_cam[:, 0] * 10)) ** 2))
        print(loss_var)  
        
        print(not torch.isnan(loss_shape).any())
        print(not torch.isnan(loss_keypoints).any())
        print(not torch.isnan(loss_keypoints_3d).any())
        print(not torch.isnan(loss_regr_pose).any())
        print(not torch.isnan(loss_regr_betas).any())
        print(not torch.isnan(loss_cam).any())
        print(not torch.isnan(log_variational_posterior).any())
        print(not torch.isnan(log_prior).any())
        '''
        loss_dict = {
            ...
            'loss/loss_var': loss_var,
            'loss/loss_cam_t': loss_cam_t,
            'loss/loss_smpl_keypoints': loss_smpl_keypoints
        }

        #for l in loss_dict:
        #    print(l + ": " + str(loss_dict[l]))
        ...
        tensor(0.0061, device='cuda:0', grad_fn=<MulBackward0>)
        tensor(8.8195e-09, device='cuda:0', grad_fn=<MeanBackward0>)
